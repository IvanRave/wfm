define(['jquery', 'ko', 'app/datacontext', 'bootstrap-modal.min', 'app/app-helper', 'app/models/ColumnAttribute'
    ...
    function WellFile(data, well) {
        ...
        self.importWellFileToLog = function () {
            ...
            datacontext.getColumnAttributes(urlQueryParams).done(function (result) {
                ...
                function submitFunction() {
                    var selectedArray = $(innerDiv).find('.pd-parameter').map(function (el) {
                        return $(this).val();
                    }).get();
        ...
        self.importToProductionData = function () {
            ...
            datacontext.getWellFiles(urlQueryParams).done(function (response) {
                        ...
                        if (cellText !== null && cellText !== '') {
                            if (j === 0 && isFileExtensionTxt === false) {
                                ...
                                if (parseInt(cellText, 10) % 1 === 0) {
                                    // it is integer
                                    cellText = moment(fromOAtoJS(parseInt(cellText, 10))).format("YYYY-MM-DD");
                                }
                            }
                ...
                for (var k = 0; k < matchSelectList.length; k++) {
                    $(matchSelectList[k].matchNameElem).on("change", { elemColumnIndex: k }, function (event) {
                        var choosedColumnName = $(this).val();
